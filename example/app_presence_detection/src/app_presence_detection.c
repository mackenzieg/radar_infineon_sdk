...
static void check_env()
{
    ...
    if(msyscon) {
        if(strncmp(msyscon, "mintty", 6) == 0)
        {
            fprintf(stderr, "Warning: mintty terminal is not recommended for this application.\n" \
    ...
    static void sendCommand(char *s)
    {
                              ...
                              ifx_PresenceSensing_Config_t* ps_config)
{
    ...
    uint32_t adc_samplerate_hz = mjson_find_number(js, strlen(js), "$.IFX_ADC_SAMPLERATE_HZ", IFX_ADC_SAMPLERATE_HZ);
    uint8_t bgt_tx_power = mjson_find_number(js, strlen(js), "$.IFX_BGT_TX_POWER", IFX_BGT_TX_POWER);
    uint8_t  rx_antenna_number = mjson_find_number(js, strlen(js), "$.IFX_RX_ANTENNA_NUMBER", IFX_RX_ANTENNA_NUMBER);
    int8_t if_gain_dB = mjson_find_number(js, strlen(js), "$.IFX_IF_GAIN_DB", IFX_IF_GAIN_DB);  

    ifx_Error_t ret = ifx_presencesensing_get_device_config(&metrics,
                                                            frame_rate,
                                                            adc_samplerate_hz,
                                                            bgt_tx_power,
                                                            rx_antenna_number,
                                                            if_gain_dB,
                                                            &ps_config->device_config);
    if(ret != IFX_OK)
    {
        return ret;
    }

    // read algo configuration params
    ps_config->mti_weight = mjson_find_number(js, strlen(js), "$.IFX_MTI_WEIGHT", IFX_MTI_WEIGHT);
    if((ps_config->mti_weight > IFX_MAX_MTI_WEIGHT)||(ps_config->mti_weight < IFX_MIN_MTI_WEIGHT))
    {
        fprintf(stderr, "presence detection configuration error: mti weight invalid! Valid values between %f - %f \n", IFX_MIN_MTI_WEIGHT, IFX_MAX_MTI_WEIGHT);
    ...
    if((ps_config->minimum_detection_range_m > metrics.maximum_range_m)||(ps_config->minimum_detection_range_m < IFX_MIN_MAXIMUM_RANGE_M))
    {
        fprintf(stderr, "presence detection configuration error: minimum detection range invalid! Valid values between %f (m) and configured maximum range %f (m)\n", IFX_MIN_MAXIMUM_RANGE_M, metrics.maximum_range_m);
        return IFX_ERROR_APP;
    }      
    ...
    if((ps_config->maximum_detection_range_m > metrics.maximum_range_m)||(ps_config->maximum_detection_range_m < IFX_MIN_MAXIMUM_RANGE_M))
    {
        fprintf(stderr, "presence detection configuration error: minimum detection range invalid! Valid values between %f (m) and configured maximum range %f (m)\n", IFX_MIN_MAXIMUM_RANGE_M, metrics.maximum_range_m);
        return IFX_ERROR_APP;
    }    
    ps_config->range_hysteresis_percentage =  mjson_find_number(js, strlen(js), "$.IFX_RANGE_HYSTERESIS", IFX_RANGE_HYSTERESIS);
    if((ps_config->range_hysteresis_percentage > IFX_MAX_RANGE_HYST_PERCENT)||(ps_config->range_hysteresis_percentage < IFX_MIN_RANGE_HYST_PERCENT))
    {
        fprintf(stderr, "presence detection configuration error: range hysteresis invalid! Valid values between %d - %d \n", IFX_MIN_RANGE_HYST_PERCENT, IFX_MAX_RANGE_HYST_PERCENT);
        return IFX_ERROR_APP;
    }       
    ps_config->absence_confirm_count = mjson_find_number(js, strlen(js), "$.IFX_ABSENCE_CONFIRM_COUNT", IFX_ABSENCE_CONFIRM_COUNT);
    if((ps_config->absence_confirm_count > IFX_MAX_ABSENCE_CONF_COUNT)||(ps_config->absence_confirm_count < IFX_MIN_ABSENCE_CONF_COUNT))
    {
        fprintf(stderr, "presence detection configuration error: absence confirm count invalid! Valid values between %d - %d \n", IFX_MIN_ABSENCE_CONF_COUNT, IFX_MAX_ABSENCE_CONF_COUNT);
        return IFX_ERROR_APP;
    } 
    ps_config->presence_confirm_count = mjson_find_number(js, strlen(js), "$.IFX_PRESENCE_CONFIRM_COUNT", IFX_PRESENCE_CONFIRM_COUNT);
    if((ps_config->presence_confirm_count > IFX_MAX_PRESENCE_CONF_COUNT)||(ps_config->presence_confirm_count < IFX_MIN_PRESENCE_CONF_COUNT))
    {
        fprintf(stderr, "presence detection configuration error: presence confirm count invalid! Valid values between %d - %d \n", IFX_MIN_PRESENCE_CONF_COUNT, IFX_MAX_PRESENCE_CONF_COUNT);
        return IFX_ERROR_APP;
    }     

    //internally hardcoded config params
...
static void wrong_usage(char * reason) {
    puts(reason);
    puts(usage_str);
    exit(EXIT_FAILURE);
}
...
Cmd_Line_Options_t decode_cmd_line_options(int argc, char** argv)
{
    Cmd_Line_Options_t opt = {0};
    while(argc > 1) {
        argc--; argv++;
...
static char* get_matrix_from_file(FILE* f , ifx_Matrix_R_t* matrix)
{
    char temp[24];
    char *cur_pos;
    for(int i=0; i < matrix->rows; i++) {
        for(int j=0; j < matrix->columns; j++) {
            do {
                ...
                if(cur_pos == NULL)
                    return NULL;
            } while((strlen(cur_pos) <= 2) && isspace(*cur_pos));
...
static void print_matrix_to_file_r(FILE* f, ifx_Matrix_R_t* matrix)
{
    ifx_Float_t cur_val;
    for(int i = 0; i < matrix->rows; i++){
        for(int j=0; j< matrix->columns; j++) {
...
int main(int argc, char** argv)
{
    ...
    Cmd_Line_Options_t opt = decode_cmd_line_options(argc, argv);
    ...
    else
    {
    	...
    	ifx_device_create_frame(1, ps_config.device_config.num_chirps_per_frame,
    ...
    ret = ifx_device_destroy_frame(&frame);
    IFX_ERR_CHECK(ret);
