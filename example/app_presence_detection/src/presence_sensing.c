                                       ...
                                       ifx_PresenceSensing_Handle_t** handle)
{
    ...
    h = ifx_mem_calloc(1, sizeof(ifx_PresenceSensing_Handle_t));
    ...
    ret = ifx_vector_create_r(config->range_fft_size, &h->range_spectrum_data.fft_spectrum_result);
    IFX_ERR_CHECK_RETURN(ret);

    ret = ifx_matrix_create_c(config->device_config.num_chirps_per_frame,
    ...
    ret = ifx_vector_create_r(doppler_fft_window_config.size, &h->doppler_fft_window);
    IFX_ERR_CHECK_RETURN(ret);
    ...
    ret = ifx_vector_create_c(config->device_config.num_chirps_per_frame, &h->doppler_data.fft_data);
    IFX_ERR_CHECK_RETURN(ret);

    ret = ifx_fft_create(FFT_TYPE_C2C, config->device_config.num_chirps_per_frame, config->doppler_fft_size, &h->doppler_fft_handle);
    IFX_ERR_CHECK_RETURN(ret);

    ret = ifx_vector_create_c(config->device_config.num_chirps_per_frame, &h->doppler_data.prepro_result);
    IFX_ERR_CHECK_RETURN(ret);

    ret = ifx_vector_create_c(config->doppler_fft_size, &h->doppler_data.chirp_fft_result);
    IFX_ERR_CHECK_RETURN(ret);

    ret = ifx_vector_create_r(config->doppler_fft_size, &h->doppler_data.abs_result);
    IFX_ERR_CHECK_RETURN(ret);
    ...
    ret = ifx_vector_create_r(h->range_spectrum_data.fft_spectrum_result.length / 2, &h->mti_result);
    IFX_ERR_CHECK_RETURN(ret);
...
ifx_Error_t ifx_presencesensing_destroy(ifx_PresenceSensing_Handle_t* handle)
{
    ...
    ret = ifx_vector_destroy_r(&handle->range_spectrum_data.fft_spectrum_result);
    IFX_ERR_CHECK(ret);

    ret = ifx_matrix_destroy_c(&handle->range_spectrum_data.frame_fft_half_result);
    IFX_ERR_CHECK(ret);

    // doppler data
    ret = ifx_vector_destroy_r(&handle->doppler_fft_window);
    IFX_ERR_CHECK(ret);

    ret = ifx_vector_destroy_c(&handle->doppler_data.fft_data);
    IFX_ERR_CHECK(ret);

    ret = ifx_fft_destroy(handle->doppler_fft_handle);
    IFX_ERR_CHECK(ret);

    ret = ifx_vector_destroy_c(&handle->doppler_data.prepro_result);
    IFX_ERR_CHECK(ret);

    ret = ifx_vector_destroy_c(&handle->doppler_data.chirp_fft_result);
    IFX_ERR_CHECK(ret);

    ret = ifx_vector_destroy_r(&handle->doppler_data.abs_result);
    IFX_ERR_CHECK(ret);

    ret = ifx_matrix_destroy_r(&handle->doppler_data.range_doppler_map);
    ...
    ret = ifx_vector_destroy_r(&handle->mti_result);
    IFX_ERR_CHECK(ret);

    ret = ifx_range_spectrum_destroy(handle->range_spectrum_handle);
    IFX_ERR_CHECK(ret);
                                    ...
                                    ifx_PresenceSensing_Handle_t* handle)
{
    ...
    ifx_vector_copy_r(&handle->range_spectrum_data.fft_spectrum_result, 0,
    ...
    for (uint32_t i = 0; i < fine_peak_result.peak_count; ++i)
    {
        ...
        {
            handle->doppler_data.prepro_result.data[i] = ifx_complex_mul_scalar(handle->doppler_data.prepro_result.data[i],
                                                                                handle->doppler_fft_window.data[i] * handle->doppler_window_scale); // Complex = Complex * (Real * Real)
        }
        ...
        if(handle->doppler_data.range_doppler_map.columns != 0) {
            ret = ifx_matrix_set_row_vector_r(&handle->doppler_data.range_doppler_map, i, (const ifx_Vector_R_t*) &handle->doppler_data.abs_result);
            IFX_ERR_CHECK_RUNTIME(ret);
        }
                                              ...
                                              ifx_PresenceSensing_Result_t* result)
{
    ...
    ret = ifx_matrix_create_c(handle->range_spectrum_data.frame_fft_half_result.rows,
                              handle->range_spectrum_data.frame_fft_half_result.columns,
                              &result->range_spectrogram);
    IFX_ERR_CHECK_RETURN(ret);

    ret = ifx_matrix_create_r(MAX_NUM_OF_TARGETS,
    ...
    ret = ifx_matrix_create_r(MAX_NUM_OF_TARGETS,
                              handle->doppler_data.abs_result.length,
                              &handle->doppler_data.range_doppler_map);
    return ret;
}
                                           ...
                                           ifx_Device_Config_t* device_config)
{
    ...
    //sanity checks
    if((metrics->maximum_range_m < IFX_MIN_MAXIMUM_RANGE_M)||(metrics->maximum_range_m > IFX_MAX_MAXIMUM_RANGE_M))
    {
            fprintf(stderr, "device configuration error: maximum range invalid! Valid values between %f - %f (m)\n", IFX_MIN_MAXIMUM_RANGE_M, IFX_MAX_MAXIMUM_RANGE_M);
            return IFX_ERROR_APP;
    }    
    if((metrics->range_resolution_m < IFX_MIN_RANGE_RESOLUTION_M)||(metrics->range_resolution_m > IFX_MAX_RANGE_RESOLUTION_M))
    {
            fprintf(stderr, "device configuration error: range resolution invalid! Valid values between %f - %f (m)\n", IFX_MIN_RANGE_RESOLUTION_M, IFX_MAX_RANGE_RESOLUTION_M);
            return IFX_ERROR_APP;
    }  
    if((metrics->maximum_speed_m_s < IFX_MIN_MAXIMUM_SPEED_M_S)||(metrics->maximum_speed_m_s > IFX_MAX_MAXIMUM_SPEED_M_S))
    {
            fprintf(stderr, "device configuration error: maximum speed invalid! Valid values between %f - %f (m)\n", IFX_MIN_MAXIMUM_SPEED_M_S, IFX_MAX_MAXIMUM_SPEED_M_S);
            return IFX_ERROR_APP;
    }      
    if((metrics->speed_resolution_m_s < IFX_MIN_SPEED_RESOLUTION_M_S)||(metrics->speed_resolution_m_s > IFX_MAX_SPEED_RESOLUTION_M_S))
    {
            fprintf(stderr, "device configuration error: speed resolution invalid! Valid values between %f - %f (m)\n", IFX_MIN_SPEED_RESOLUTION_M_S, IFX_MAX_SPEED_RESOLUTION_M_S);
            return IFX_ERROR_APP;
    }      


    if(metrics->maximum_range_m < (8 * metrics->range_resolution_m))
    {
            fprintf(stderr, "device configuration error: maximum range must be at least 8 times range resolution!\n");
            return IFX_ERROR_APP;
    }
    if(metrics->speed_resolution_m_s >= metrics->maximum_speed_m_s)
    {
            fprintf(stderr, "device configuration error: speed resolution must be lesser than maximum speed!\n");
            return IFX_ERROR_APP;
    }    
     ...
     */
    device_config->num_samples_per_chirp = (uint32_t) (2.0f * metrics->maximum_range_m / metrics->range_resolution_m);
    ...
    device_config->chirp_to_chirp_time_100ps = (uint64_t) (1.0e10 * lambda / (double)(4.0f * metrics->maximum_speed_m_s));
     ...
     * rounding was found here: https://graphics.stanford.edu/~seander/bithacks.html#RoundUpPowerOf2.
     */
    device_config->num_chirps_per_frame = (uint32_t) (2.0f * metrics->maximum_speed_m_s / metrics->speed_resolution_m_s);
...
ifx_Error_t ifx_presencesensing_get_device_config(const ifx_PresenceSensing_Metrics_t* metrics,
                                                  ...
                                                  ifx_Device_Config_t* device_config)
{
    ifx_Error_t ret = ifx_presencesensing_tranlate_metrics_to_config(metrics, device_config);

    device_config->frame_period_us = (uint64_t) (1.0e6f / frame_rate);
    device_config->adc_samplerate_hz = adc_samplerate_hz;
    device_config->bgt_tx_power = bgt_tx_power;
    device_config->rx_antenna_mask = (0x01 << (rx_antenna_number - 1));
    device_config->if_gain_dB = if_gain_dB;
