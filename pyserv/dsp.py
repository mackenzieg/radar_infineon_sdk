...
class RadarDSP:
    ...
    def process_packet(self, json_data):
        ...
        self.circular_buff[:, self.circular_buff.shape[1] - 1] = complex_mti

        #plt.figure(1)
        #plt.plot(self.r_xvals, complex_abs)
        #plt.draw()

        plt.figure(2)
        plt.plot(self.st_xvals, np.absolute(self.circular_buff[self.important_bin]))
        plt.draw()
